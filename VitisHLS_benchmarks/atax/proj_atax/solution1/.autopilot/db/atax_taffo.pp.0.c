...
__attribute__((sdx_kernel("atax", 0))) void atax(float* A, float* x, float* y)
{
#line 55 "/home/bruno/Desktop/benchmarks/atax/run_hls_1.tcl"
#pragma HLSDIRECTIVE TOP name=atax
# 25 "atax_taffo.c"
...
#pragma HLS INTERFACE m_axi port = y depth = 16
 int m = 16;
    ...
    VITIS_LOOP_43_1: for (i = 0; i < 16; i++){
        y_true[i] = zero;
    }

    VITIS_LOOP_47_2: for (i = 0; i < 16; i++)
    {

      tmp = zero;

      VITIS_LOOP_52_3: for (j = 0; j < 16; j++){

        tmp = tmp + As[i*16 +j] * xs[j];
      }
      VITIS_LOOP_56_4: for (j = 0; j < 16; j++){
    ...
    VITIS_LOOP_63_5: for (j = 0; j < 16; j++){
        y[j] =(float) y_true[j];
      }
}
...
int main(int argc, char** argv)
{
  ...
  float A[m*n] __attribute__((annotate("target('A') scalar(range(1, 4))")));
    ...
    VITIS_LOOP_97_1: for (i = 0; i < 16; i++){
     tmp_x = one;
        x[i] = tmp_x;
    }

    VITIS_LOOP_102_2: for (i = 0; i < 16; i++){

        VITIS_LOOP_104_3: for (j = 0; j < n; j++){
        tmp_A = ((i+j)%n) / (five*m);
          A[i*n+j] = one;
        }
    }
    VITIS_LOOP_109_4: for (i = 0; i < 16; i++){
        y[i] = zero;
        y_true[i] = zero;
    }

    float tmp __attribute__((annotate("scalar(range(-4, 4))")));
    VITIS_LOOP_115_5: for (i = 0; i < 16; i++)
    {
      tmp = zero;

      VITIS_LOOP_119_6: for (j = 0; j < 16; j++){
        tmp = tmp + A[i*16 +j] * x[j];
      }
      VITIS_LOOP_122_7: for (j = 0; j < 16; j++){
  ...
  VITIS_LOOP_130_8: for (i = 0; i < 16; i++){
        printf("%0.16f \n", (float)y[i]);
        printf("%f \n", y_true[i]);
    }
  return 0;
