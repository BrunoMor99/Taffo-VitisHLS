...
{
#line 44 "/home/bruno/Desktop/benchmarks/atax/run_hls_no_taffo.tcl"
...
#pragma HLS INTERFACE m_axi port = y depth = 16
 int m = 16;
    ...
    VITIS_LOOP_48_1: for (i = 0; i < 16; i++)
    {
#pragma HLS PIPELINE II=1

 tmp = zero;
      VITIS_LOOP_53_2: for (j = 0; j < 16; j++){
#pragma HLS PIPELINE II=1
 tmp = tmp + As[i*16 +j] * xs[j];
      }

      VITIS_LOOP_58_3: for (j = 0; j < 16; j++){
...
int main(int argc, char** argv)
{
  ...
  float A[m*n];
    ...
    VITIS_LOOP_95_1: for (i = 0; i < 16; i++){
     tmp_x = one + (i / fn);
        x[i] = tmp_x;
    }

    VITIS_LOOP_100_2: for (i = 0; i < 16; i++){

        VITIS_LOOP_102_3: for (j = 0; j < n; j++){
        tmp_A = ((i+j)%n) / (five*m);
          A[i*n+j] = tmp_A;
        }
    }
    VITIS_LOOP_107_4: for (i = 0; i < 16; i++){
        y[i] = zero;
    }
  ...
  VITIS_LOOP_114_5: for (j = 0; j < 16; j++){
        printf("%f \n", y[j]);
        printf("%f \n", y[j]);
    }
  return 0;
