...
entity doitgen_control_s_axi is
generic (
...
end entity doitgen_control_s_axi;
...
architecture behave of doitgen_control_s_axi is
    ...
    constant ADDR_C_0_DATA_0 : INTEGER := 16#1c#;
    constant ADDR_C_0_DATA_1 : INTEGER := 16#20#;
    constant ADDR_C_0_CTRL   : INTEGER := 16#24#;
    constant ADDR_SUM_DATA_0 : INTEGER := 16#28#;
    constant ADDR_SUM_DATA_1 : INTEGER := 16#2c#;
    constant ADDR_SUM_CTRL   : INTEGER := 16#30#;
    ...
    signal int_sum             : UNSIGNED(63 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_SUM_DATA_1 =>
                        rdata_data <= RESIZE(int_sum(63 downto 32), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_SUM_DATA_1) then
                    int_sum(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_sum(63 downto 32));
                end if;
            end if;
        end if;
    end process;
