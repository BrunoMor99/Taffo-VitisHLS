...
__attribute__((sdx_kernel("doitgen", 0))) void doitgen(float* A , float* C, float* sum)
{
#line 55 "/home/bruno/Desktop/benchmarks/doitgen/run_hls_1.tcl"
#pragma HLSDIRECTIVE TOP name=doitgen
# 25 "doitgen_taffo.c"
...
#pragma HLS ARRAY_PARTITION variable=C type=block factor=4 dim=1
 int nr = 16;
  ...
  VITIS_LOOP_50_1: for (r = 0; r < 16; r++){
#pragma HLS PIPELINE II=1
 VITIS_LOOP_52_2: for (q = 0; q < 16; q++) {
#pragma HLS PIPELINE II=1
 VITIS_LOOP_54_3: for (p = 0; p < 16; p++) {
#pragma HLS unroll factor=8
 sums[p] = 0.0;
     VITIS_LOOP_57_4: for (s = 0; s < 16; s++){
#pragma HLS PIPELINE II=1
 sums[p] += As[r*nq+q*np+s] * Cs[s * 16 + p];
      ...
      VITIS_LOOP_64_5: for (p = 0; p < 16; p++){
#pragma HLS unroll factor=8
...
int main(int argc, char** argv)
{
  ...
  VITIS_LOOP_93_1: for (i = 0; i < nr; i++){
    VITIS_LOOP_94_2: for (j = 0; j < nq; j++){
      VITIS_LOOP_95_3: for (k = 0; k < np; k++){
     A[i*nq+j*np+k] = ((i*j + k)%np) / np;
       }
    }
  }
  VITIS_LOOP_100_4: for (i = 0; i < np; i++){
    VITIS_LOOP_101_5: for (j = 0; j < np; j++){
      C[i*np+j] = ((i*j) %np) / np;
    }
   }
