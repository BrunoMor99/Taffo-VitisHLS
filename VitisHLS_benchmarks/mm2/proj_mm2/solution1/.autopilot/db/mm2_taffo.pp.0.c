...
__attribute__((sdx_kernel("mm2", 0))) void mm2(float* A, float* B, float* C, float* D, float alpha, float beta)
{
#line 55 "/home/bruno/Desktop/benchmarks/mm2/run_hls_1.tcl"
#pragma HLSDIRECTIVE TOP name=mm2
# 24 "mm2_taffo.c"
...
#pragma HLS INTERFACE m_axi port = D depth = 16*16
 int ni = 16;
  ...
  Ds = D;
  float tmp[16*16] __attribute__((annotate("tartget('tmp') scalar(range(-16384, 16384))")));
  ...
  VITIS_LOOP_53_1: for (i = 0; i < 16; i++){

    VITIS_LOOP_55_2: for (j = 0; j < 16; j++){

     tmp[i*nj+j] = 0.0;
     VITIS_LOOP_58_3: for (k = 0; k < 16; ++k){
  ...
  VITIS_LOOP_65_4: for (i = 0; i < 16; i++){

    VITIS_LOOP_67_5: for (j = 0; j < 16; j++){

 Ds[i*nl+j] *= betas;
 VITIS_LOOP_70_6: for (k = 0; k < 16; ++k){
...
int main(int argc, char** argv)
{
  ...
  VITIS_LOOP_102_1: for (i = 0; i < ni; i++){
    VITIS_LOOP_103_2: for (j = 0; j < nk; j++){
        A[i*nk+j] = ((i*j+1) % ni) / ni;
    }
  }

  VITIS_LOOP_108_3: for (i = 0; i < nk; i++){
    VITIS_LOOP_109_4: for (j = 0; j < nj; j++){
        B[i*nj+j] = (i*(j+1) % nj) / nj;
    }
  }

  VITIS_LOOP_114_5: for (i = 0; i < nj; i++){
    VITIS_LOOP_115_6: for (j = 0; j < nl; j++){
        C[i*nl+j] = ((i*(j+3)+1) % nl) / nl;
    }
  }

  VITIS_LOOP_120_7: for (i = 0; i < ni; i++){
    VITIS_LOOP_121_8: for (j = 0; j < nl; j++){
