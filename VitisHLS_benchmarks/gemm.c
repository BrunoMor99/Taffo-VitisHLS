...
#pragma HLS INTERFACE m_axi port = D depth = NI*NJ
    ...
    for (ii = 0; ii < NI; ii++)
    {
        ...
        for (jj = 0; jj < NJ; jj++)
        {
            ...
            #pragma HLS pipeline
                tmp[ii*NJ+jj] += alpha * A[ii*NK+k] * B[k*NJ+jj];
            }
...
int main(){
    ...
    int nk = NK;
    int nl = NL;
    double A[NI*NK] __attribute__((annotate("scalar()"))); 
    double B[NK*NJ] __attribute__((annotate("scalar()"))); 
    double C[NJ*NL] __attribute__((annotate("scalar()"))); 
    double D[NI*NJ] __attribute__((annotate("target('D') scalar(range(-16384, 16384) final)"))); 
