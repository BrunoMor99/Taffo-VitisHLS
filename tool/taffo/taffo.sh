...
if [[ $bc -eq 1 ]]; then
  ...
  # Output .ll file (default behavior)
  output_extension=".ll"
fi
...
if [[ ${#input_files[@]} -eq 1 ]]; then
  ...
  ${CLANG} \
    ...
    -c -emit-llvm \
    ${input_files} \
    -S -o "${temporary_dir}/${output_basename}.1.taffotmp${output_extension}" || exit $?       
else
  # > 1 input files
  tmp=()
  for input_file in "${input_files[@]}"; do
    ...
    thisfn=${thisfn%.*}
    thisfn="${temporary_dir}/${output_basename}.${thisfn}.0.taffotmp${output_extension}"
    tmp+=( $thisfn )
    ${CLANG} \
      $opts -D__TAFFO__ -O0 -Xclang -disable-O0-optnone $compat_flags_clang \
      -c -emit-llvm \
...
build_float="${AUTO_CLANGXX} $opts ${optimization} ${float_opts} $compat_flags_clang ${temporary_dir}/${output_basename}.1.taffotmp${output_extension}"
...
${OPT} \
  ...
  ${init_flags} \
  -S -o "${temporary_dir}/${output_basename}.2.taffotmp${output_extension}" "${temporary_dir}/${output_basename}.1.taffotmp${output_extension}" || exit $?
...
if [[ $disable_vra -eq 0 ]]; then
  ${OPT} \
    ...
    -S -o "${temporary_dir}/${output_basename}.3.taffotmp${output_extension}" "${temporary_dir}/${output_basename}.2.taffotmp${output_extension}" || exit $?;
else
  cp "${temporary_dir}/${output_basename}.2.taffotmp${output_extension}" "${temporary_dir}/${output_basename}.3.taffotmp${output_extension}";
fi
...
while [[ $feedback_stop -eq 0 ]]; do
  ...
  ${OPT} \
    ...
    -S -o "${temporary_dir}/${output_basename}.4.taffotmp${output_extension}" "${temporary_dir}/${output_basename}.3.taffotmp${output_extension}" || exit $?
  ...
  ${OPT} \
    ...
    -S -o "${temporary_dir}/${output_basename}.5.taffotmp${output_extension}" "${temporary_dir}/${output_basename}.4.taffotmp${output_extension}" || exit $?
  ...
  if [[ ( $enable_errorprop -eq 1 ) || ( $feedback -ne 0 ) ]]; then
    ${OPT} \
      ...
      -S -o "${temporary_dir}/${output_basename}.6.taffotmp${output_extension}" "${temporary_dir}/${output_basename}.5.taffotmp${output_extension}" 2> "${temporary_dir}/${output_basename}.errorprop.taffotmp.txt" || exit $?
  ...
  ${build_float} -S -emit-llvm \
    -o "${temporary_dir}/${output_basename}.float.taffotmp${output_extension}" || exit $?
  ${TAFFO_PE} \
    --fix "${temporary_dir}/${output_basename}.5.taffotmp${output_extension}" \
    --flt "${temporary_dir}/${output_basename}.float.taffotmp${output_extension}" \
...
if [[ ( $emit_source == "s" ) || ( $del_temporary_dir -eq 0 ) ]]; then
  if [[ $cuda_kern == 1 ]]; then
    ...
    -c \
    "${temporary_dir}/${output_basename}.5.taffotmp${output_extension}" \
...
if [[ $emit_source == "s" ]]; then
  ...
  cp "${temporary_dir}/${output_basename}.5.taffotmp${output_extension}" "$output_file${output_extension}"
  if [[ $bc -eq 1 ]]; then
  	clang -emit-llvm -c "$output_file${output_extension}" -o "$output_file"
  fi
else
  ${AUTO_CLANGXX} \
    $opts ${optimization} $compat_flags_clang \
